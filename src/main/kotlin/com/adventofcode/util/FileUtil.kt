...
fun readTwoColumnsOfNumbers(fileName: String, splitBy: String): Pair<List<Int>, List<Int>> {
...
fun readLinesOfNumbers(fileName: String, splitBy: String): List<List<Int>> {
    ...
    File(fileUrl.toURI()).forEachLine { line ->
        val numbers = line.trim().split(splitBy.toRegex()).map { it.toInt() }
        list.add(numbers)
    }
